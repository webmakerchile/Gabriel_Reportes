...
- *Semántica preservada:* regla OBU-{id}, "el primer rut gana", rut inválido en update mantiene el existente, `activo` desde `cliente_activo`, cartera respeta desactivaciones manuales, `api_obuma_ids` se sigue recolectando pero (igual que antes) NO se marca inactivo a los ausentes del API. Tests: `tests/test_sync_clientes_bulk.py` (16 tests con sqlite in-memory + savepoints reales).
