...
- Lock de sync (`_sync_report_lock` en `src/reports/excel_generator.py`): `sync_for_report` adquiere un lock de módulo no-bloqueante para que dos jobs que coinciden a las 06:30 (p.ej. lunes: diario + cobranza) no sincronicen en paralelo y dupliquen ventas (no hay constraint único en `ventas_historico`).
